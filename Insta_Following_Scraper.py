...
class Following_Parser(object):
    def __init__(self,login_user,login_password,target,count,Timeout):
        ...
        """
        count 는 원하는 상대 팔로잉 수를 보고 맞춰주세요
        """
        ...
        WebDriverWait(self.client, Timeout).until(EC.presence_of_element_located((By.XPATH, '/html/body/div[1]/div/div/div/div[1]/div/div/div/div[1]/section/main/div/header/section/ul/li[3]/a'))).click()


        self.lst = []
        for i in range(1,count+1):
            self.following = WebDriverWait(self.client, Timeout).until( EC.presence_of_element_located((By.XPATH, f'/html/body/div[1]/div/div/div/div[1]/div/div/div/div[1]/section/main/div[2]/div/div/div[{i}]/div[2]/div[1]')))
            self.real_following = self.following.text

            print(f"[SUCCESS] - {self.real_following}")
            sleep(0.5)

            self.client.execute_script("arguments[0].scrollIntoView();", self.following)

            sleep(1)

            self.lst.append(self.real_following)

        print("Following Load Finish!")
